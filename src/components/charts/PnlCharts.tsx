...
function TooltipBody({ args, field }: { args: TooltipArgs; field: "pnl" | "cumulative" }) {
  ...
  return (
    <div style={tooltipStyle.contentStyle} className="px-3 py-2">
      <p style={tooltipStyle.labelStyle}>{String(label)}{p.live ? " · inclui o valor em direto" : ""}</p>
...
/** Gain or loss of each period: green above zero, red below, the bar with live quotes outlined. */
export function DailyPnlBars({ data, height = 260 }: { data: DailyPoint[]; height?: number }) {
  if (!data.length) return <p className="py-8 text-center text-sm text-ink-3">Sem registos suficientes. É preciso pelo menos dois valores das carteiras no período.</p>;
  return (
