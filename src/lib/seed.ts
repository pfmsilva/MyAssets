...
type RuleDef = string | { pattern: string; priority: number };
const CATEGORIES: { name: string; kind: CategoryKind; color: string; rules: RuleDef[] }[] = [
  { name: "Salário", kind: "INCOME", color: "#008300", rules: ["VENCIMENTO", "ORDENADO", "SALARIO", "SALÁRIO", "REMUNERACAO"] },
  { name: "Outros rendimentos", kind: "INCOME", color: "#1baf7a", rules: ["JUROS", "DIVIDENDO", "Dividend", "Free funds interest", "REEMBOLSO IRS", "SEG SOCIAL", "ABONO"] },
...
export async function runSeed(prisma: PrismaClient) {
  ...
  for (const [i, c] of CATEGORIES.entries()) {
    ...
    const defs = c.rules.map((r) => (typeof r === "string" ? { pattern: r, priority: c.kind === "EXPENSE" ? 0 : 10 } : r));
    const missing = defs.filter((r) => !existingRules.has(r.pattern.toLowerCase()));
    if (missing.length) {
      created.rules += missing.length;
      await prisma.categoryRule.createMany({ data: missing.map((r) => ({ pattern: r.pattern, categoryId: cat.id, priority: r.priority })) });
    }
  }
  return created;
}
